...
podido realizar la operación y false en caso contrario.
20*/
    public boolean metodoPrestamo (HashSet<Libro> h1, Libro libro){
               
        //recorrer conjuntos para analizar atributos de un elemento solo con for each
        for (Libro aux : h1) {
            if (aux.equals(libro)) {
                if (aux.getNumEjemplares()>=1) {
    ...
    public void mostrarBiblioteca (HashSet<Libro> h1){
        System.out.println("");
        for (Libro aux : h1) {
...
realizar la operación y false en caso contrario.*/
    ...
    public boolean metodoDevolucion (HashSet<Libro> h1, Libro libro){
               
        //recorrer conjuntos para analizar atributos de un elemento solo con for each
        for (Libro aux : h1) {
            if (aux.equals(libro)) {
                if (aux.getNumPrestados()>=1) {
                    System.out.println("El libro fue devuelto");
                    aux.setNumEjemplares(aux.getNumEjemplares()+1);
                    aux.setNumPrestados(aux.getNumPrestados()-1);
                    return true;
                }else{
                    System.out.println("No se han prestado libros con este titulo");
                }
            }
        }
